...
export const nextjsGenerateMultiple = new Command("generate-multiple")
     .description("Generate CRUD operations for multiple CSV files without overwriting the schema")
     .option("-d, --directory <filePath>", "Specify an output directory for generated files.")
     .option("-p, --pattern <pattern>", "CSV file pattern to match (default: *.csv)", "*.csv")
     .argument("[csvDirectory]", "Directory containing CSV files (default: current directory)", ".")
     .action(async (csvDirectory, opts) => {
          try {
               ...
               console.log("2. Run 'npm install @prisma/client prisma' to install Prisma");

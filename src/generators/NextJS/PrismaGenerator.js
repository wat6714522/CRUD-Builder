...
export default class NextJS {
  ...
  async Initialize() {
    ...
    this.fields = this.csvData.fields.filter((field) =>
      this.isValidFieldName(field),
    );
  ...
  async GeneratePrismaSchema() {
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.info(
        `✅ Created new schema with ${this.EntityName.toPascalCase} model`,
      );
    }
  }
      ...
      .map((field) => {
        ...
        switch (type) {
          case "string":
            ...
            break;
          case "number":
            prismaType = sampleValue?.toString().includes(".")
              ? "Float"
              : "Int";
            break;
          case "boolean":
...
datasource db{
  ...
  async GeneratePrismaClientManager() {
    ...
    if (fs.existsSync(prismaClientFilePath)) {
      console.info(
        `ℹ️  Prisma client manager already exists at: ${prismaClientFilePath}`,
      );
      return;
    }
    ...
    console.info(
      `✅ Generated Prisma client manager at: ${prismaClientFilePath}`,
    );
  }
}
